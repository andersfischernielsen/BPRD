...
let allocate (kind : int -> var) (typ, x) (varEnv : varEnv) : varEnv * instr list =
    ...
    | TypA (t, Some i) ->
      ...
      (newEnv, code)
    | TypR (t, b, s, e) ->
      let length = (int) (Operators.ceil ((double)((e-b)/s)))
      let newEnv = ((x, (kind (fdepth+length), typ)) :: env, fdepth+length+1)

      let rec createSteps current step acc =
        if current <= 0
        then CSTI step :: acc
        else createSteps (current-1) (step+s) (CSTI step :: acc)
      let generated = List.rev (createSteps length s [])

      let code = generated @ [GETSP; CSTI (length-1); SUB]
      (newEnv, code)
    | _ ->
